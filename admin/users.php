...
if( isset($_GET['add'])){
	...
	$errors = array();
	if($_POST){
		$qryEmail = $db->query("SELECT * FROM users WHERE email = '$email'");
		$emailCount = mysqli_num_rows($qryEmail);
		if($emailCount!=0){
	...
	$qryUser = $db->query("SELECT * FROM users ORDER BY full_name");
	?>
	...
	<table class="table table-bordered table-striped table-condensed">
		...
		<tbody>
			<?php while( $user = mysqli_fetch_assoc($qryUser)) :?>
			<tr>
				<td>
